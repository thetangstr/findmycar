#!/usr/bin/env python3
"""
Migrate existing vehicle data to the new SQLite schema with standardized attributes
"""
...
from database_v2_sqlite import VehicleV2, Base, get_database_url, init_db
...
class VehicleMigrator:
    ...
    def migrate_all(self, batch_size: int = 100):
        ...
        # Update search text for all vehicles
    ...
    def migrate_vehicle(self, v1_vehicle: VehicleV1):
        ...
        try:
            ...
            raw_data = {
                ...
                'image_urls': v1_vehicle.image_urls,
                **vehicle_details
            }
            ...
            inferred_data = self.inference_engine.infer_attributes({
                'title': v1_vehicle.title,
            ...
            v2_vehicle = VehicleV2(
                ...
                exterior_color=standardized['core_fields'].get('exterior_color') or v1_vehicle.exterior_color,
                interior_color=standardized['core_fields'].get('interior_color') or v1_vehicle.interior_color,
                transmission=standardized['core_fields'].get('transmission') or inferred_data.get('transmission') or v1_vehicle.transmission,
                drivetrain=standardized['core_fields'].get('drivetrain') or inferred_data.get('drivetrain') or v1_vehicle.drivetrain,
                fuel_type=standardized['core_fields'].get('fuel_type') or inferred_data.get('fuel_type') or v1_vehicle.fuel_type,
    ...
    def _convert_image_urls(self, vehicle: VehicleV1) -> List[str]:
        ...
        # Check if image_urls already exists
        if hasattr(vehicle, 'image_urls') and vehicle.image_urls:
            if isinstance(vehicle.image_urls, list):
                return vehicle.image_urls
            elif isinstance(vehicle.image_urls, str):
                try:
                    urls = json.loads(vehicle.image_urls)
                except:
    ...
    def update_search_text(self):
        """Update search text for all vehicles (SQLite full-text search)"""
        logger.info("Updating search text...")
        
        # For SQLite, concatenate searchable fields
        vehicles = self.v2_session.query(VehicleV2).all()
        ...
        for vehicle in vehicles:
            ...
            search_parts = [
                ...
                str(vehicle.year or ''),
                str(vehicle.title or ''),
                str(vehicle.description or ''),
                str(vehicle.body_style or ''),
                str(vehicle.exterior_color or ''),
                str(vehicle.transmission or ''),
                str(vehicle.fuel_type or '')
            ]
            
            vehicle.search_text = ' '.join(search_parts).lower()
        
        try:
            self.v2_session.commit()
            logger.info("Search text updated successfully")
        except Exception as e:
            logger.error(f"Error updating search text: {e}")
            self.v2_session.rollback()
